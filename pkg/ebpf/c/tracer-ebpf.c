...
static void update_tcp_stats(conn_tuple_t* t, tcp_stats_t stats) {
    ...
    if (stats.ktime_established > 0) {
        val->ktime_established = stats.ktime_established;
    }
}
...
static int handle_retransmit(struct sock* sk, tracer_status_t* status) {
    ...
    tcp_stats_t stats = {.retransmits = 1, .rtt = 0, .rtt_var = 0, .ktime_established = 0 };
...
int kprobe__tcp_set_state(struct pt_regs* ctx) {
    ...
    tcp_stats_t stats = { .ktime_established = bpf_ktime_get_ns(), .retransmits = 0, .rtt = 0, .rtt_var = 0 };
    update_tcp_stats(&t, stats);
